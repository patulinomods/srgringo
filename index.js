			...
			switch(command) {
				...
				case 'sticker':
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						reply(`Envie fotos com legendas ${prefix}sticker ou responda de imagem que já foram enviadas`)
					}
					break
				case 'gtts':
				...
				case 'loli':
					loli.getSFWLoli(async (err, res) => {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'aki esta'})
					})
					break
				case 'nsfwloli':
				...
				case 'hilih':
					...
					break
				case 'yt2mp3':
					if (args.length < 1) return reply('Oque voce procura?')
				...
				case 'att':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `[ Esta é uma msg do sr gringo e pato ]\n\n${body.slice(4)}`})
						}
						reply('Sucesso')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `[ Esta é uma msg do sr gringo e pato ]\n\n${body.slice(4)}`)
						}
        ...
        case 'promover':
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]} agora e um adm\n`
						}
						mentions(from, mentioned, true)
						client.groupRemove(from, mentioned)
					} else {
						mentions(`ok @${mentioned[0].split('@')[0]} (se o bot n for adm n vai pegar ne garaio)`, mentioned, true)
				...
				case 'rebaixar':
					...
					if (mentioned.length > 1) {
						...
						mentions(`nao foi possivel tirar adm @${mentioned[0].split('@')[0]} pois eu n sou adm!`, mentioned, true)
				...
				case 'adminlista':
					if (!isGroup) return reply(mess.only.group)
					teks = `lista de ademe *${groupMetadata.subject}*\nTotal : ${groupAdmins.length}\n\n`
					no = 0
					...
					break
				case 'toimg':
					if (!isQuotedSticker) return reply('❌Marque uma figurinha com esse comando❌')
					reply(mess.wait)
				...
				case 'simi':
					...
					if (Number(args[0]) === 0) {
						if (isSimi) return reply('Modo bot interativo ativado')
						samih.push(from)
